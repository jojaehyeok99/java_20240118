...
// 전화걸기 : call - power가 true일때 (상대 전화번호)로 전화를 겁니다.
...
class TriangleMath {
	...
	//static final double PI = 3.14; // 단일 책임원칙이 지켜지지 않은 것 삼각형 관련 class에서 pi는 관련 없음
	
	static double getHypotenuse (int bottom, int height) {
		if (bottom <=0 || height <= 0) return 0; // 매개변수 유효성 검사 : 매개변수가 적절한 값으로 입력이 되었는지 확인하는 작업
		// if (!(bottom > 0 && height > 0)) // 매개변수 유효성 검사
		double result = Math.pow(bottom, 2) + Math.pow(height, 2); //math.pow : 제곱해주는 math 클래스
		return Math.sqrt(result); //math.sqrt : 제곱근해주는 math 클래스
	}
	
	static double getCircumference (int height, int bottom, int hypotenuse) {
		if (hypotenuse <=0 || height <= 0 || bottom <= 0) return 0; // 매개변수 유효성 검사
...
public class A_Example {
	...
	public static void main(String[] args) {
		...
		// TriangleMath.PI = 0.0; -> 상수로 선언해서 값 변경을 하지 못함
		// 단일 책임원칙이 지켜지지 않음 삼각형 관련 class에서 pi는 관련 없음
