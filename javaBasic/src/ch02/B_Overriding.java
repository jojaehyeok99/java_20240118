...
// 규칙
// 1. 상위클래스에서 선언된 메서드와 메서드 이름, 매개변수의 조합이 같아야 함
// 2. 상위클래스에서 선언된 메서드가 final로 선언되면 재정의 불가능
// 3. 상위클래스에서 선언된 메서드보다 더 좁은 범위의 접근 제어를 할 수 없음
...
class SubClass2 extends SuperClass1{
	...
	void instanceMethod () {
		...
		//System.out.println("조상 클래스의 메서드입니다."); -> 다음 값이 변경할때 찾아가서 변경해야되는 번거로움이 있음
