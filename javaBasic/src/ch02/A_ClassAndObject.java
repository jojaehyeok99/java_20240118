...
class ExampleClass1 {
	...
	// [접근제어자 일반제어자] 반환타입 메서드명 (매개변수타입 매개변수명[,...]) --> 메서드 선언부
	// { 메서드 기능 구현 } --> 메서드 구현부
	...
	void method3 (int arg1) {
		//void에서의 return은 강제로 메서드를 종료하기 위해 사용 - break의 기능
		return;
	}
	...
	// 클래스와 같은 이름을 가지고 있고, 반환타입이 존재하지 않음 -> 무조건 인스턴스 타입
	...
	ExampleClass1 (int attribute1, double attribute2) {
		...
		this.attribute1 = attribute1; // this 뒤에 attribute는 위에서 선언한 변수 / = 뒤에 attribute 매개변수(외부에서 받아오는 값)
	...
	// 오버로딩 조건
	// 1. 메서드 이름을 중복해서 작성하지만 매개변수의 타입 및 개수가 다르게 작성
	// 2. 메서드의 반환타입과 메서드의 매개변수의 이름은 오버로딩에 영향을 미치지 않음
		...
		// 91 - 95 : 매개변수 개수 95 - 99 : 매개변수 타입 103 - 107 : 매개변수 배치
		
//		int overloadMethod(double arg1, int arg2) {
//			return 0;
//		}
		// 107 - 112 : int의 영향이 미치지 않음
		
//		void overloadMethod(double double1, int int2) {
//			
//		}
		// 107 - 117 : int number double number이 안되듯이 위에것도 안됨
