...
public class D_Control {
	...
	public static void main(String[] args) {
		...
		// 제어문 : 조건에 따라서 코드의 흐름을 결정하는 문
		
		Scanner scanner = new Scanner(System.in);
		
		// 조건문 : 특정 조건이 만족하면 코드를 실행하는 것, 만족하지 않으면 실행하지 않음
		
		// if문 : 주어진 조건이 true이면 코드를 실행하도록 하는 것
		// 문법 : if (조건-논리표현식) { 실행할 코드 }
...
//		System.out.println("프로그램 종료");
		...
		// 부정 연산자 : 논리표현식을 부정하는 연산
		// !논리: true이면 false, false면 true
...
//		System.out.println("프로그램 종료");
		...
		// else문 : if 문의 조건-논리표현식이 false이면 코드를 실행하도록 하는 것
		// 문법 : if(조건-논리표현식) { true이면 실행할 코드}
		// 		 else {false이면 실행할 코드}
		
		// ** 주의해야 될점
		// 1. else문은 반드시 if문 이후에 와야한다. (else 단독으로 사용이 불가능)
...
//				
//		}
		// else를 빼는 경우 검사를 계속 진행, else를 넣는 경우 검사 후 걸러서 진행
		
		//-------------------------------------------//
		
		// switch문 : 특정 변수 또는 값에 따라 실행할 코드 블록의 범위를 결정하는 것
//문법 : 	switch (변수-일반표현식){
//		case 값1 :
//			변수-일반표현식이 값1과 같을때 시작할 코드위치
//		case 값2 :
//			변수-일반표현식이 값2과 같을때 시작할 코드위치
//			swich 내부에서 break문을 만나면 switch문을 종료
//			break;
//		case 값3 :
//			변수-일반표현식이 값3과 같을때 시작할 코드위치
//		default :
...
//		}
		...
		// for문 : 일반적 반복 횟수가 정해져 있을 때 사용하는 반복문
		// 문법 : for(조건에사용할변수초기화; 조건; 조건에서사용할변수증감){ 반복할 코드 }
...
//			System.out.println(numbers[index]);
//		}
		// 길이(length) 포함하지 않는 범위 <
		
		// ---------------------------------------------------//
		
		// forEach문 : 컬렉션(배열)의 각 요소를 하나씩 변수에 '복사'하여 사용하는 반복문
...
//		while (input != 0) {
//			input = scanner.nextInt();
//			if(input % 2 !=0) { //2의 배수만 출력하고 싶을때
//				continue; // continue : 반복문에서 continue 문을 만나면 조건 검사로 넘어감
//			}
//			if(input == 0) {
